...
for i, country in enumerate(countries):
    ...
    axs[i].plot(data["Year"], data["Score"],"royalblue", marker='>')
    axs[i].set_title(country)
    axs[i].grid(True)
    axs[i].set_xticks(xticks_years)

plt.tight_layout()
plt.suptitle("WJP Score Over Time", fontsize=16, y=1.02)
plt.xlabel("Year")
plt.ylabel("WJP Score (0-100)")
plt.show()
