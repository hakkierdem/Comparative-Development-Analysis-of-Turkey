...
df = pd.read_excel("general_score_scimagojr.xlsx")
...
for i, country in enumerate(countries):
    ...
    axs[i].plot(data["Year"], data["scientific_score"],"g", marker='v',)
    axs[i].set_title(country)
    axs[i].grid(True)
    axs[i].set_xticks(xticks_years)
    
plt.tight_layout()
plt.suptitle("Scimagojr Score Over Time", fontsize=16, y=1.02)
plt.xlabel("Year")
plt.ylabel("Scimagojr Score (0-100)")
plt.show()
